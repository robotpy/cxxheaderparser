...
class CxxParser:
    ...
    def _merge_attributes(
        self, current: typing.List[Attribute], new: typing.Iterable[Attribute]
    ) -> typing.List[Attribute]:
        if new:
            current.extend(new)
        return current

    def _attributes_to_value(self, tokens: LexTokenList) -> Value:
        return Value([Token(tok.value, tok.type) for tok in tokens])

    def _split_attribute_list(self, tokens: LexTokenList) -> typing.List[LexTokenList]:
        if not tokens:
            return []

        groups: typing.List[LexTokenList] = []
        current: LexTokenList = []
        depth = 0
        opens = {"(", "[", "{", "<"}
        closes = {")", "]", "}", ">"}

        for tok in tokens:
            if tok.type in opens:
                depth += 1
            elif tok.type in closes:
                depth -= 1

            if tok.type == "," and depth == 0:
    ...
    def _parse_single_attribute(
        self, tokens: LexTokenList
    ) -> typing.Optional[Attribute]:
        if not tokens:
            return None

        name_tokens: typing.List[LexToken] = []
        value_tokens: typing.List[LexToken] = []
        saw_parens = False
        depth = 0
        ...
        value = self._attributes_to_value(value_tokens) if saw_parens else None
    ...
    def _parse_attribute_list_tokens(
        ...
        attrs = []
        for group in self._split_attribute_list(tokens):
            attr = self._parse_single_attribute(group)
            if attr:
                attrs.append(attr)
        return attrs
    ...
    def _consume_gcc_attribute(
        ...
        *,
        ...
        while True:
            ...
            tokens = self._consume_balanced_tokens(tok1, tok2)
            attrs.extend(self._parse_attribute_list_tokens(tokens[2:-2]))
    ...
    def _consume_declspec(
        ...
        *,
        ...
        attrs = self._parse_attribute_list_tokens(tokens[1:-1])
        if record:
            self._record_attributes(attrs)
        return attrs
        ...
        *,
        ...
        while True:
            if tok.type == "DBL_LBRACKET":
                tokens = self._consume_balanced_tokens(tok)
                attrs.extend(self._parse_attribute_list_tokens(tokens[1:-1]))
            elif tok.type == "alignas":
                next_tok = self._next_token_must_be("(")
                tokens = self._consume_balanced_tokens(next_tok)
                attrs.append(
                    Attribute("alignas", self._attributes_to_value(tokens[1:-1]))
                )
            else:
                self.lex.return_token(tok)
                break
    ...
    def _parse_declarations(
        ...
        attributes = self._merge_attributes(attributes, self._take_pending_attributes())
